...
suffix_tree::suffix_tree(const std::vector<std::string>& input_strs) :
  m_strs(input_strs), m_root_ptr(new suffix_tree_node()), m_id_pool(0) {

  this->m_root_ptr->m_parent_ptr = this->m_root_ptr;
  this->m_root_ptr->m_suffix_link_ptr = this->m_root_ptr;

  for (size_t i = 0; i < this->m_strs.size(); ++i) {
    this->add_string(i);
  }
}
...
std::pair<size_t, std::vector<size_t>> suffix_tree::get_lrs(size_t str_id) {
  ...
  const std::function<void(suffix_tree_node*)> find_deepest_internal = [&find_deepest_internal, &deepest_internal, str_id](suffix_tree_node* cur_ptr) -> void {
    ...
    if (cur_ptr->is_visited_by(str_id) && cur_ptr->m_child_ptr && cur_ptr->m_depth > deepest_internal->m_depth) {
...
std::pair<size_t, std::pair<std::vector<size_t>, std::vector<size_t>>> suffix_tree::get_lrs(size_t str_one_id, size_t str_two_id) {
  suffix_tree_node* deepest_shared_ptr = this->m_root_ptr;
  const std::function<void(suffix_tree_node*)> find_deepest_shared_ptr = [
    &find_deepest_shared_ptr,
    &deepest_shared_ptr,
    str_one_id,
    str_two_id
  ](suffix_tree_node* cur_ptr) -> void {
    if (!cur_ptr) {
      return;
    }
    find_deepest_shared_ptr(cur_ptr->m_child_ptr);
    find_deepest_shared_ptr(cur_ptr-> m_next_sib_ptr);
    if (
      cur_ptr->is_visited_by(str_one_id) &&
      cur_ptr->is_visited_by(str_two_id) &&
      cur_ptr->m_child_ptr &&
      cur_ptr->m_depth > deepest_shared_ptr->m_depth
    ) {
      deepest_shared_ptr = cur_ptr;
    }
  };
  find_deepest_shared_ptr(this->m_root_ptr);
  std::vector<size_t> str_one_occurences;
  std::vector<size_t> str_two_occurences;
  return {
    deepest_shared_ptr->m_depth,
    {
      this->get_lrs_starts(deepest_shared_ptr, str_one_id),
      this->get_lrs_starts(deepest_shared_ptr, str_two_id)
    }
  };
}
  ...
  const std::function<void(suffix_tree_node*)> bwt_dfs = [&bwt_dfs, &bwt_str, &str_id, this](suffix_tree_node* cur_ptr) -> void {
    if (!cur_ptr) {
      ...
      if (cur_ptr->m_id == 0) {
        ...
        bwt_str.push_back(this->m_strs[str_id][cur_ptr->m_id - 1]);
      }
      return;
    }
...
void suffix_tree::add_string(const size_t string_idx) {
  ...
  #endif
  for (const char* cur_char_ptr = this->m_strs[string_idx].c_str(); *cur_char_ptr != '\0'; ++cur_char_ptr) {
    #if USE_NAIVE_ALG
...
suffix_tree_node* suffix_tree::find_path_and_insert(suffix_tree_node* cur_ptr, const char* cur_char_ptr, const size_t string_id) {
  ...
  if (k == cur_ptr->m_size) {
    ...
    if (next_ptr) {
      ...
      new_leaf_ptr->m_owner = string_id;
      new_leaf_ptr->m_visitors.insert(string_id);
      new_leaf_ptr->m_id = this->m_id_pool++;
      new_leaf_ptr->m_start = j;
      new_leaf_ptr->m_size = this->get_string_size(string_id) - (j - this->m_strs[string_id].c_str());
...
suffix_tree_node* suffix_tree::split_edge(suffix_tree_node* cur_ptr, size_t size, const size_t string_id) {
  ...
  new_internal_ptr->m_id = cur_ptr->m_owner;
...
std::vector<size_t> suffix_tree::get_lrs_starts(suffix_tree_node* common_ptr, size_t str_id) {
  std::vector<size_t> lrs_starts;
  ...
  if (!found_flag) {
    lrs_starts.push_back(this->m_strs[str_id].size() - common_ptr->m_depth);
  }
  return lrs_starts;
}
...
void execute_single_comparison(
  ...
  const size_t a = lrs_info.first;
  const size_t b = alignment::execute_modified_global_alignment(prefix_one, prefix_two, 1, -2, -5, -2);
  const size_t c = alignment::execute_modified_global_alignment(suffix_one, suffix_two, 1, -2, -5, -2);
  *result_dest = a + b + c;
}
